...
public class UDPEchoServer {
	...
	private static void validateAttributes(int portNumber, int bufferSize) {
		if(portNumber <= 1024) throw new IllegalArgumentException("O servidor UDP n o pode usar portar reservadas.");
		if(bufferSize < MIN_BUFFER_SIZE) throw new IllegalArgumentException(String.format("O buffer de mensagem precisa ser maior que % d ", MIN_BUFFER_SIZE));
	}
	...
	private static void run() throws IOException {
		...
		while(true) {
			received = receive();
			if(dataIsUnderSize(received)) reply(received);
			else {
